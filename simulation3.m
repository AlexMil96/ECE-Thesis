...
disp(type);
% answer=questdlg('Select scheduling algorithm','Scheduling','Round Robin','Proportional Fair','Best CQI','Round Robin');
% switch answer
%     case 'Round Robin'
%         scheduling="Round Robin";
%     case 'Proportional Fair'
%         answer2=questdlg('Select type of Proportional Fair','Proportional Fair','version 1','version 2','version 1');
%         switch answer2
%             case 'version 1'
%                 scheduling="Proportional Fair without starvation limit";
%             case 'version 2'
%                 scheduling="Proportional Fair with starvation limit";
%         end
%     case 'Best CQI'
%         scheduling="Best CQI";
% end
...
radius = PathLoss(Pt,type,0,0); %(Pt,type,d,value)   bazo d=value=0; epeidi thelo na brw radius, type=1 urban,type=2 suburban 
...
plot(c_x1,c_y1,'r+');
hold on;
coord=subs(c_x1,c_y1,radius,UEspercell);
...
while sumoutside ~= 6*n
    ...
    if (cx>=-frame/2) && (cx<=frame/2) && (cy>=-frame/2) && (cy<=frame/2)
        ...
        plot(cx,cy,'r+');
        hold on;
        coord=subs(cx,cy,radius,UEspercell);
        centers(i,1)=cx;
        centers(i,2)=cy;
        i=i+1;
        for j=1:UEspercell
...
[Long_Term_SINR,positions,r0,r,Losses]=LTSINR(Pt,type,centers,coordinates); %epistrefei linear sinr
...
for k=1:tti
    for q=1:totalUEs
        for i=1:totaleNBs
            ...
            %% Hbar matrix (2x4)
%             if k==1     %loop arxikopoihshs tou Hbar(t=H0)
%                 if centers(i,1)==positions(q,3) && centers(i,2)==positions(q,4)
%                     var = 10^(-PathLoss(Pt,type,r0(q,1),1)/10);    %PL tou q-UE me ton eNB tou cell tou
%                 elseif centers(i,1)~=positions(q,3) || centers(i,2)~=positions(q,4)
%                     var = 10^(-PathLoss(Pt,type,r(q,i),1)/10);  %PL tou q-UE me eNB diaforetikou cell
%                 end
%                 %Hbar{2,q,i} = sqrt(1/2)*(randn(Nr,Nt)+1i*randn(Nr,Nt)); 
%                 Hbar{q,i} = sqrt(1/2)*(normrnd(0,0.5,[Nr,Nt])+1i*normrnd(0,1,[Nr,Nt]));
%             end
%             if k~=1     %loop Hbar gia t~=1
%                 Hbar{q,i} = e*Hbar{1,q,i}+sqrt(1-e^2)*N{q,i};
%             end
...
%             Hbar{2,q,i}=zeros(Nr,Nt);
    ...
    for q=1:totalUEs
        ...
        for i=1:totaleNBs
%             if r(q,i)==0
%                 %tempPopt=Popt{1,q};     % to p mikro sto paper
%                 suma1 = suma1 + (H{q,i}*Popt{1,q}(:,1)) * (H{q,i}*Popt{1,q}(:,1))';
%                 suma2 = suma2 + (H{q,i}*Popt{1,q}(:,2)) * (H{q,i}*Popt{1,q}(:,2))';
%             elseif r(q,i)~=0
%                 n=randi(16);
...
%             end
            ...
            if r(q,i)~=0   %q user den anikei sto i eNB
                n=randi(16);
                %suma1 = suma1 + (H{q,i}*P{n,1}(:,1)) * (H{q,i}*P{n,1}(:,1))';
                %suma1 = suma1 + (H{q,i}*Popt{1,q}(:,1)) * (H{q,i}*Popt{1,q}(:,1))';
                %suma2 = suma2 + (H{q,i}*P{n,1}(:,2)) * (H{q,i}*P{n,1}(:,2))';
                %suma2 = suma2 + (H{q,i}*Popt{1,q}(:,2)) * (H{q,i}*Popt{1,q}(:,2))';
                sumb = sumb + (H{q,i}*P{n,1})*(H{q,i}*P{n,1})';
            end
        end
    ...
    for q=1:totalUEs
        for i=1:totaleNBs
            if r(q,i)==0
                ...
                %SINR1(k,q) = abs((H{k,q,i}*tempPopt(:,1))' * (Rn{q,2}^(-1)) * (H{k,q,i}*tempPopt(:,1)));
                %SINR2(k,q) = abs((H{k,q,i}*tempPopt(:,2))' * (Rn{q,1}^(-1)) * (H{k,q,i}*tempPopt(:,2)));
            end
        end
    end
    j=1; i=1;
    ...
    %% CQI
    [cqi,modulation_order,coding_rate,cqisum]=CQI(sinr1_dB,sinr2_dB,UEspercell,totaleNBs,k,cqisum);
    for i=1:totaleNBs
        for j=1:UEspercell
    ...
    if sectoring==0
        ...
        [temp1,temp2,temp3,allocation,rrcounter]=RRsim3(sectoring,k,totaleNBs,UEspercell,temp1,temp2,temp3,allocation,rrcounter,modulation_order,coding_rate);
        ...
        [temp2,temp1,temp3,allocation,metricPF1,numPF1,denPF1]=PF6sim3(sectoring,temp2,k,UEspercell,totaleNBs,totalRBs,temp1,temp3,modulation_order,coding_rate,allocation,metricPF1,numPF1,denPF1);
        ...
        [temp2,temp1,temp3,allocation,metricPF2,numPF2,denPF2,starvcounter]=PF6v2sim3(type,sectoring,temp2,k,UEspercell,totaleNBs,totalRBs,temp1,temp3,modulation_order,coding_rate,allocation,metricPF2,numPF2,denPF2,starvcounter);
        ...
        [temp2,temp1,temp3,allocation] =bestcqi2sim3(sectoring,temp2,k,UEspercell,totaleNBs,totalRBs,temp1,temp3,modulation_order,coding_rate,allocation);
    ...
    elseif sectoring==1
        ...
        [temp1,temp2,temp3,allocationsc,rrcounter]=RRsim3(sectoring,k,totalsectors,UEspersector,temp1,temp2,temp3,allocationsc,rrcounter,modulation_ordersc,coding_ratesc);
        ...
        [temp2,temp1,temp3,allocationsc,metricscPF1,numscPF1,denscPF1]=PF6sim3(sectoring,temp2,k,UEspersector,totalsectors,totalRBs,temp1,temp3,modulation_ordersc,coding_ratesc,allocationsc,metricscPF1,numscPF1,denscPF1);
        ...
        [temp2,temp1,temp3,allocationsc,metricscPF2,numscPF2,denscPF2,starvcountersc]=PF6v2sim3(type,sectoring,temp2,k,UEspersector,totalsectors,totalRBs,temp1,temp3,modulation_ordersc,coding_ratesc,allocationsc,metricscPF2,numscPF2,denscPF2,starvcountersc);
        ...
        [temp2,temp1,temp3,allocationsc] =bestcqi2sim3(sectoring,temp2,k,UEspersector,totalsectors,totalRBs,temp1,temp3,modulation_ordersc,coding_ratesc,allocationsc);
...
%         y=UEspersector(1,s+2);
%         z=UEspersector(1,s+3);
%         usage(1:x,i)=usagesc(1:x,s+1);
%         usage(x+1:x+y,i)=usagesc(1:y,s+2);
%         usage(x+y+1:x+y+z,i)=usagesc(1:z,s+3);
%         for k=1:tti
%             instant_throughput(1:x,i,k)=instant_throughputsc(1:x,s+1);
%             instant_throughput(x+1:x+y,i,k)=instant_throughputsc(1:y,s+2);
%             instant_throughput(x+y+1:x+y+z,i,k)=instant_throughputsc(1:UEspersector(1,s+3),s+3);
%             usagepertti{k,1}(1:x,i)=usageperttisc{k,1}(1:x,s+1);
%             usagepertti{k,1}(x+1:x+y,i)=usageperttisc{k,1}(1:y,s+2);
%             usagepertti{k,1}(x+y+1:x+y+z,i)=usageperttisc{k,1}(1:z,s+3);
%             for m=1:totalRBs
...
% numberofzeros=zeros(1,totaleNBs);
% for i=1:totaleNBs
%     for j=1:UEspercell
%         if usage(j,i)==0
%             numberofzeros(1,i)=numberofzeros(1,i)+1;
%         end
%     end
% end

% numberoftimes=zeros(tti,totaleNBs); 
% for i=1:totaleNBs    %doylevei mono an exw perissotera UEs apo RB!!!!!!!!!!!!!!!!!
%     for j=1:UEspercell
%         for times=1:tti
%             if usage(j,i)==times
%                 numberoftimes(times,i)=numberoftimes(times,i)+1;
%                 break;
%             end
%         end
%     end
...
% testforones=zeros(1,totaleNBs);
% for i=1:totaleNBs
%     for j=1:UEspercell
%         if usage(j,i)==1
%             testforones(1,i)=testforones(1,i)+1;
%         end
%     end
% end
...
%% graphs
% avg_thr_per_tti=zeros(tti,1);
% temp=zeros(UEspercell,totaleNBs);
% for k=1:tti
%     temp=instant_throughput(:,:,k);
%     avg_thr_per_tti(k,1)=sum(sum(temp))/totalUEs;
%     avg_thr_per_tti(k,1)=avg_thr_per_tti(k,1)*1000*(10^(-6));
% end
